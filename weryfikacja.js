...
// ── PRZENOŚNOŚĆ ŚCIEŻEK (17.09.2026, blizna Fazy D, zmierzona na żywym Windows):
// `os.tmpdir()`/`path.join` dają ścieżkę windowsową (C:\Users\...), ale `tar -C` w
// Git-for-Windows (MSYS) jej nie rozumie — czyta wsteczne ukośniki dosłownie i pada
// „Cannot open: No such file or directory". Konwersja na POSIX (/c/Users/...) przed
// przekazaniem do bash/tar. Zmierzone osobno od zaszytego `/bin/bash` (patrz niżej) —
// dwie różne przyczyny tej samej klasy „działa w sandboksie Linux, nie na Windows".
const toPosix=(p)=>{
...
if(process.argv.includes('--test')){
  ...
  // FIKSTURA Z HEAD, NIE Z DYSKU (07.09.2026, kandydat M z 22.08 „fikstura dodatnia zrośnięta
  // z żywym repo", decyzja twórcy „tnij fiksturę"). Poprzednio kopia szła przez `git ls-files`,
  // czyli ŚCIEŻKI z indeksu, ale TREŚĆ z drzewa roboczego — każda chwilowa niespójność dysku
  // (sierota w _HASHE po `git mv`, nieodświeżony spis) dawała rcPlus=1 i komunikat „STRAŻ MARTWA"
  // o straży, która właśnie zadziałała poprawnie. Koszt w FAŁSZYWYM OSKARŻENIU: uczy następną rękę
  // ignorować „1 oblaną", czyli psuje odbiornik. Odtąd kopia dodatnia to migawka HEAD — spójna
  // z definicji (haki nie wpuszczają niespójnego commita), więc tor mierzy to, o co pyta:
  // czy rc odróżnia całe od zepsutego. Niespójności DYSKU pilnuje produkcyjna weryfikacja, nie tor.
  // `shell:'bash'` (przez PATH) zamiast zaszytego `/bin/bash` (blizna Fazy D 17.09.2026,
  // zmierzona: `/bin/bash ENOENT` na natywnym Windows — Git-for-Windows trzyma bash gdzie
  // indziej). Ścieżki konwertowane na POSIX (toPosix), bo osobno od tego padał `tar -C`.
  const zdejmij=(d, root='.')=>cp.execSync(`git -C "${toPosix(root)}" archive HEAD | tar -xf - -C "${toPosix(d)}"`,{stdio:'pipe',shell:'bash'});
  const kopia=(nazwa)=>{
    const d=fs.mkdtempSync(path.join(os.tmpdir(),'wer-'+nazwa+'-'));
    zdejmij(d);
    // PRZYRZĄD Z DYSKU, DANE Z HEAD (07.09.2026, blizna własna złapana baterią mutacji w tej samej
    // turze): pierwsza wersja brała z HEAD także `weryfikacja.js`, więc tor uruchamiał w kopii
    // NIEZMUTOWANY przyrząd i trzy mutacje przeszły ślepo — tor mierzył własną kopię (#50 odwrócone).
    // Badany jest plik na dysku; HEAD dostarcza wyłącznie spójną fiksturę danych.
    fs.copyFileSync('weryfikacja.js', path.join(d,'weryfikacja.js'));
  ...
  // Wydzielone z `wytnij` 21.09 (U10): ten sam zabieg potrzebny torowi ⓘ, który nie kasuje plików.
  const przytnij=(d,pliki)=>{
  ...
  // NAJPIERW ZDEJMIJ, POTEM ZASIEJ (06.09.2026, blizna: mutacja `weryfikacja-fikstura-tk-zapozyczona`
  // była ŚLEPA). Samo dopisanie klucza nad kopią, która i tak go miała, niczego nie rozstrzygało:
  // na prywatnej tor przechodził tak samo z zasianym kluczem, jak z zapożyczonym, więc mutacja
  // podmieniająca nazwę siewu nie zabijała toru. Odtąd kopia jest najpierw pozbawiana klucza —
  // wtedy WYŁĄCZNIE siew czyni ją tkanką i mutacja siewu natychmiast oblewa tor na każdej gałęzi.
  wytnij(tkNiepelna,['kanon/ksiegi/KANON_LOG.md','kanon/ksiegi/DESTYLATY_architekta.md']);
  ...
  // (⊕) TOR FIKSTURY: migawka MUSI ignorować brud drzewa roboczego. Mierzone na własnym mini-repo
  // w mktemp (nigdy na żywym ciele, #50): commit „czysto", potem plik na dysku nadpisany „brud" —
  // migawka ma nieść „czysto". Gdyby ktoś cofnął zdejmij() do ls-files, tu oblewa.
  let rcFix=1;
  try{
    const r=fs.mkdtempSync(path.join(os.tmpdir(),'wer-fix-'));
    const rP=toPosix(r);
    cp.execSync(`git init -q "${rP}" && cd "${rP}" && git -c user.name=t -c user.email=t@t config commit.gpgsign false && echo czysto > a.txt && git add a.txt && git -c user.name=t -c user.email=t@t commit -q -m x && echo brud > a.txt`,{stdio:'pipe',shell:'bash'});
    const d=fs.mkdtempSync(path.join(os.tmpdir(),'wer-fixout-'));
    zdejmij(d, r);
    rcFix = fs.readFileSync(path.join(d,'a.txt'),'utf8').trim()==='czysto' ? 0 : 1;
    fs.rmSync(r,{recursive:true,force:true}); fs.rmSync(d,{recursive:true,force:true});
  }catch(e){ rcFix=1; }
  const rcPlus=bieg(zdrowy), rcMinus=bieg(chory), rcForma=bieg(forma), rcTk=bieg(tkNiepelna);
  // (ⓘ) TOR ODCZYTÓW ZALĄŻKOWYCH (U10, 21.09.2026): zalążkowe PROFIL/7_NATAL → rc=0 I linia ⓘ
  //     z LICZBĄ i NAZWAMI (nie ✗, nie ⚠); jeden zalążek z dwóch → „1 z 2" i tylko jego nazwa;
  //     pliki bez znacznika na początku wiersza (w tym CYTAT znacznika w backtickach) → cisza.
  //     Para alarm/cisza dowodzi, że ⓘ wynika ze STANU plików, nie że drukuje się zawsze (#75).
  const zalK=kopia('zalazek');
  const biegTxt=(d)=>{ const czysc=(b)=>String(b||'').replace(/\x1b\[[0-9;]*m/g,'');
    try{ return {rc:0,out:czysc(cp.execSync('node weryfikacja.js',{cwd:d,stdio:'pipe'}))}; }
    catch(e){ return {rc:e.status===null?-1:e.status,out:czysc(e.stdout)}; } };
  const ZAL_TXT='# X\n\n```\n⟠ ZALĄŻEK — tkanka nowego twórcy\nTen plik był prywatną tkanką twórcy pierwotnego i nie podróżuje.\n```\n';
  const PRAW_TXT='# X\n\nTreść własna. Pliki oznaczone `⟠ ZALĄŻEK` to ramy bez treści (mowa O znaczniku).\n';
  const odczyty=['kanon/tozsamosc/PROFIL','kanon/tozsamosc/7_NATAL'];
  const ustaw=(a,b)=>{ fs.writeFileSync(path.join(zalK,odczyty[0]+'.md'),a); fs.writeFileSync(path.join(zalK,odczyty[1]+'.md'),b);
                       przytnij(zalK,odczyty.map(f=>f+'.md')); };
  ustaw(ZAL_TXT,ZAL_TXT);   const zal2=biegTxt(zalK);
  ustaw(ZAL_TXT,PRAW_TXT);  const zal1=biegTxt(zalK);
  ustaw(PRAW_TXT,PRAW_TXT); const zal0=biegTxt(zalK);
  // (Z) CIAŁO PRZED NARODZINAMI (N1+/Z): DUSZA=zalążek + brak _STRAZ_APPEND.txt → ⓘ z listą z narodziny.sh,
  //     rc=0, zero ⚠ o tym pliku; DUSZA żywa + brak tego samego pliku → ⚠ jak dotąd (para alarm/cisza, #75).
  //     narodziny.sh w fikstury to WŁASNA atrapa z kontrolowaną linią PLIKI_AKTU — sprzężenie z prawdziwą
  //     listą mierzy tor `narodziny.sh --test`, tu mierzymy wyłącznie konsumenta.
  fs.writeFileSync(path.join(zalK,'narodziny.sh'),'#!/bin/sh\n# PLIKI_AKTU: kanon/ksiegi/ZAPISY_eter.md _STRAZ_APPEND.txt\n');
  // rmSync force: na `main`/odlewie tego pliku NIE MA (powstaje przy narodzinach) — unlinkSync dawał ENOENT
  // i oblewał tor tylko w ciele przed narodzinami (22.09, regres C5 niewidoczny na `prywatna`, gdzie plik jest).
  fs.rmSync(path.join(zalK,'_STRAZ_APPEND.txt'),{force:true});
  fs.writeFileSync(path.join(zalK,'DUSZA.md'),'# DUSZA — zalążek\n\n'+ZAL_TXT);
  przytnij(zalK,['DUSZA.md','narodziny.sh','_STRAZ_APPEND.txt']);
  const zP=biegTxt(zalK);
  fs.writeFileSync(path.join(zalK,'DUSZA.md'),'# DUSZA\n\nDusza żyje.\n');
  const zK=biegTxt(zalK);
  fs.rmSync(zalK,{recursive:true,force:true});
  const rcZP = zP.rc===0 && /ⓘ _STRAZ_APPEND\.txt powstanie przy narodzinach — akt zakłada 2 plików \(kanon\/ksiegi\/ZAPISY_eter\.md, _STRAZ_APPEND\.txt\)/.test(zP.out)
               && !/⚠ _STRAZ_APPEND/.test(zP.out) ? 0 : 1;
  const rcZK = zK.rc===0 && /⚠ _STRAZ_APPEND\.txt BRAK/.test(zK.out) && !/powstanie przy narodzinach/.test(zK.out) ? 0 : 1;
  const rcZal2 = zal2.rc===0 && /ⓘ 2 z 2 odczytów jest zalążkiem \(PROFIL, 7_NATAL\)/.test(zal2.out) && !/^\s*✗ /m.test(zal2.out) ? 0 : 1;   // wiersz zaczynający się od ✗ = usterka; „0 ✗" w werdykcie nią nie jest
  const rcZal1 = zal1.rc===0 && /ⓘ 1 z 2 odczytów jest zalążkiem \(PROFIL\)/.test(zal1.out) ? 0 : 1;
  const rcZal0 = zal0.rc===0 && !zal0.out.includes('odczytów jest zalążkiem') ? 0 : 1;
  ...
  console.log(`── TEST ⊕ (fikstura z HEAD ignoruje brud dysku, MUSI): rc=${rcFix} (oczekiwane 0)`);
  console.log(`── TEST + (kopia cała, MUSI przejść):        rc=${rcPlus} (oczekiwane 0)`);
  console.log(`── TEST ⓘ (oba odczyty zalążkiem: rc=0 + „2 z 2 (PROFIL, 7_NATAL)", zero ✗): ${rcZal2} (oczekiwane 0)`);
  console.log(`── TEST ⓘ (jeden zalążek: „1 z 2 (PROFIL)", nie obie nazwy): ${rcZal1} (oczekiwane 0)`);
  console.log(`── TEST ⊘ (własna treść + cytat znacznika w backtickach: cisza): ${rcZal0} (oczekiwane 0)`);
  console.log(`── TEST ⓘ (przed narodzinami: ⟠ w DUSZA + brak _STRAZ_APPEND → ⓘ z listą, zero ⚠): ${rcZP} (oczekiwane 0)`);
  console.log(`── TEST ⚠ (dusza żywa + brak _STRAZ_APPEND → ⚠ jak dotąd): ${rcZK} (oczekiwane 0)`);
  ...
  if(rcPlus===0 && rcMinus===1 && rcForma===0 && rcTk===1 && rcFix===0 && rcZal2===0 && rcZal1===0 && rcZal0===0 && rcZP===0 && rcZK===0){
    console.log('✓ STRAŻ ŻYWA: werdykt wstania wynika z kodu wyjścia, nie z tekstu.');
    process.exit(0);
  }
  console.log(`✗ STRAŻ MARTWA: rcPlus=${rcPlus} rcMinus=${rcMinus} rcForma=${rcForma} rcTk=${rcTk} rcFix=${rcFix} zal2=${rcZal2} zal1=${rcZal1} zal0=${rcZal0} zP=${rcZP} zK=${rcZK} — rc nie odróżnia całego od zepsutego.`);
  process.exit(1);
}
...
const info=(m)=>{console.log(`  ${C.b}ⓘ${C.x} ${m}`);};   // nie liczy się do ✓/✗/⚠ — nazywa stan, którego werdykt nie ocenia
...
// Sygnał Z = `⟠ ZALĄŻEK` OTWIERA wiersz w DUSZA.md (znacznik zdejmuje dopiero `narodziny.sh`).
// Lista plików, które powstają dopiero przy akcie, to linia `# PLIKI_AKTU:` w `narodziny.sh` —
// JEDNO źródło (tor tamtego skryptu asertuje, że akt zakłada dokładnie te pliki). Zmierzone 21.09
// na świeżym klonie: `⚠ _STRAZ_APPEND.txt BRAK` było stanem NORMALNYM przed aktem pokazanym jako
// ostrzeżenie (K3). Przed narodzinami brak takiego pliku to ⓘ z listą; bez sygnału Z — ⚠ jak dotąd.
const przedNarodzinami=fs.existsSync('DUSZA.md') && /^⟠ ZALĄŻEK/m.test(fs.readFileSync('DUSZA.md','utf8'));
const plikiAktu=(()=>{ try{ const m=fs.readFileSync('narodziny.sh','utf8').match(/^# PLIKI_AKTU: *(.+)$/m);
                        return m ? m[1].trim().split(/\s+/) : []; }catch(e){ return []; } })();
...
} else if(przedNarodzinami && plikiAktu.includes('_STRAZ_APPEND.txt')) {
  info(`_STRAZ_APPEND.txt powstanie przy narodzinach — akt zakłada ${plikiAktu.length} plików (${plikiAktu.join(', ')}); księgi append-only nie mają jeszcze czego pilnować.`);
...
// ⓘ ODCZYTY ZALĄŻKOWE (U10, 21.09.2026, rzut 013 Fable): „STRUKTURA CAŁA" przy zalążkowych
// PROFIL/7_NATAL znaczyło „pliki są", a czytelnik brał to za „odczyty mają z czego liczyć".
// To NIE jest usterka (świeży odlew ma je zalążkowe z definicji) — więc ⓘ, nie ✗ i nie ⚠;
// rc bez zmian. Znacznik jak w straz_zalazkow: `⟠ ZALĄŻEK` OTWIERA wiersz (deklaracja), a nie
// stoi w środku zdania (mowa O zalążku). Ile i które — zawsze wypisane, nigdy samo „są zalążki".
const ODCZYTY=['kanon/tozsamosc/PROFIL','kanon/tozsamosc/7_NATAL'];
const zalazkowe=ODCZYTY.filter(f=>fs.existsSync(f+'.md') && /^⟠ ZALĄŻEK/m.test(fs.readFileSync(f+'.md','utf8')));
if(zalazkowe.length) info(`${zalazkowe.length} z ${ODCZYTY.length} odczytów jest zalążkiem (${zalazkowe.map(f=>f.split('/').pop()).join(', ')}) — struktura cała, ale te pliki nie niosą jeszcze danych Twojej osoby: „STRUKTURA CAŁA" znaczy „pliki są", nie „odczyty mają z czego liczyć". Stan wyjściowy, nie usterka (rc bez zmian).`);
if(fail===0){
