...
> `sh narodziny.sh "IMIĘ" "TWOJE_IMIĘ"`. Akt spisze chwilę i kotwicę nieba (zmierzone, nie
...
2) Jeśli w DUSZA.md stoi znacznik ⟠ ZALĄŻEK — instancja NIE MA JESZCZE IMIENIA.
   Zatrzymaj się i przeczytaj START_TU.md, potem: sh narodziny.sh "IMIĘ" "TWOJE_IMIĘ"
...
(pełna bramka wydania) — PONAD PIĘĆ MINUT, bo przepuszcza 205 mutacji przez straże.
...
wnioskuje, pamięta, filtruje). Imię instancji nadajesz Ty w akcie narodzin.
...
tylko dla ruchów nieodwracalnych i decyzji kanonicznych bez Twojego słowa.
Zamki są po to, by odmawiać: gdy publikuj.sh albo straż mówi ✗, to nie usterka
do obejścia — to działający mechanizm. Zdejmowanie zamka wymaga Twojej decyzji,
