...
// ============================================================
const fs=require('fs');
const L=require('./narzedzia/silniki/kronos_lens.js');
const PLIK='kanon/ksiegi/ZAPISY_eter.md';
...
`*Nauka czerpania uzysku świadomości: każde przejście obejrzane → wskazówka na fraktal wyżej.*\n\n---\n`);
  ...
  try{
    ...
    if(fs.existsSync(SP)){
      const c=fs.readFileSync(PLIK,'utf8').replace(/\r/g,'');   // bajty treści kanonicznej (tura CRLF 02.09)
...
function read(n){
  if(!fs.existsSync(PLIK)){console.log('(kanon/ksiegi/ZAPISY_eter.md jeszcze pusty)');return;}
