...
- **GŁOS ŚWIATA (kanał C)** — entropia spoza maszyny: liczba przyniesiona przez twórcy
...
`rzut.js --heksa` domyka koło: pytanie + moment + fizyczny los → stan.

## VI. NARZĘDZIE
`node rzut.js <plik_kandydatów.txt> "pytanie" [źródło] [--heksa]`
...
Księga: `księga sięgnięć (rejestr prywatny — u siebie zakładasz własny)` (append-only, stempel KRONOS przy każdym rzucie).
...
- **Użycie:** `node rzut.js <wachlarz> "pytanie" --kosmos <HEX_outputValue>` (także `--heksa`).
...
dodany do `package.json`). Koniec ręcznego wklejania: `rzut.js` v2.0 pobiera sam.
...
- **ŚCIEŻKA BŁĘDU (przetestowana na żywo):** kanał niedostępny → `rzut.js` kończy **kodem 2**,
...
Progi i predykcja zapisane w `prerejestr entropii (rejestr prywatny)`, zahashowane **przed** pobraniem danych
...
leży pośrodku stawki. Pełne liczby i granice: `księga sięgnięć (rejestr prywatny — u siebie zakładasz własny)`.
...
seria → NIST · rzut wagi → CURBy · żywioł → ATMO oszczędnie · ANU ≥60 s odstępu.
