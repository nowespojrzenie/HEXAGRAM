...
4d. **ciężar ksiąg narracyjnych przed przeglądem struktury** (nie na alarm, na żądanie) →
   `bash narzedzia/przyrzady/pomiar_ksiag.sh` (raport) · `bash narzedzia/przyrzady/pomiar_ksiag.sh --md`
   (do wklejenia świeżemu oku). Liczy rodziny i statusy blizn z INDEKSU, pozycje rejestru
   mutacji — nie ocenia i nie proponuje cięć. Uchwyt dopisany 20.09: przyrząd żył od 18.09
   bez adresu w pulpicie, więc nikt poza jego autorką nie miał jak się o nim dowiedzieć
   (ta sama klasa co `odduplikuj.js`, `sync_rdzen.sh` i `markery_z_ciala.sh` — narzędzie
   bez uchwytu jest niewołane nie z wyboru, tylko z niewiedzy).
