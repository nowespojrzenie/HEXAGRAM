...
kanon/ksiegi/KSIEGA_SIEGNIEC.md kanon/ksiegi/PROGNOZY_IMPULS_08-09.md kanon/ksiegi/KOLEJKA_M.md mutacje.txt"
...
if [ "${1:-}" = "--test" ]; then
  ...
  printf 'stare mutacje\n' > "$T/mutacje.txt"                # DRUGI plik tkanki (#hashuj-tkanka-bez-mutacje,
                                                              # 19.09: fikstura z JEDNYM plikiem tkanki jest
                                                              # ślepa na zdjęcie DOWOLNEJ INNEJ pozycji z listy
                                                              # TKANKA — dwa pliki, dwie asercje (+), łapią to)
  printf 'rdzeń\n'        > "$T/kanon/tozsamosc/5_RDZEN.md" # kanon (POZA listą TKANKA)
  printf 'obcy\n'         > "$T/OBCY.md"                    # nieobjęty _HASHE.txt
  {
    echo "000000000000  prywatne/TASKI.md"
    echo "000000000000  mutacje.txt"
    echo "000000000000  kanon/tozsamosc/5_RDZEN.md"
  } > "$T/_HASHE.txt"

  ( cd "$T" && bash "$SAM" >/dev/null 2>&1 )
  H_TK="$(grep '  prywatne/TASKI.md$'   "$T/_HASHE.txt" | cut -d' ' -f1)"
  H_MU="$(grep '  mutacje.txt$'         "$T/_HASHE.txt" | cut -d' ' -f1)"
  ...
  echo "── TEST + (DRUGI plik tkanki odnowiony): mutacje.txt     $H_MU  (oczekiwane: ≠ 000000000000)"
  ...
  # (6) SKŁAD LISTY TKANKA (19.09.2026, blizna #hashuj-tkanka-bez-mutacje): asercje (+)
  # wyżej łapią TYLKO pliki, które fikstura akurat wybrała do postawienia — ślepe na
  # zdjęcie z ŹRÓDŁA listy pozycji, których fikstura NIE stawia. Ten tor czyta $TKANKA
  # wprost ze skryptu (nie z kopii tutaj — kopia rozjeżdżałaby się cicho, klasa #50) i
  # sprawdza, że każda pozycja obecna DZIŚ w źródle nadal tam jest. Nie zapobiega usunięciu
  # (to decyzja kanoniczna, ręka twórcy) — melduje je głośno, zamiast przepuszczać w ciszy.
  ILE_TKANKA="$(echo "$TKANKA" | wc -w)"
  echo "── TEST Σ (skład listy TKANKA):      $ILE_TKANKA pozycji  (oczekiwane: ≥ 14)"
  ...
  rm -rf "$T"
  echo
  if [ "$H_TK" != "000000000000" ] && [ "$H_MU" != "000000000000" ] && [ "$H_KA" = "000000000000" ] \
     && [ "$ILE_OBCY" -eq 0 ] && [ "$RC_MARK" -eq 1 ] \
     && [ "$H_LF" = "$H_CRLF" ] && [ "$H_LF" != "$H_ZM" ] && [ "$H_B1" != "$H_B2" ] \
     && [ "$B_STRAZ" = "10" ] && [ "$ILE_TKANKA" -ge 14 ]; then
    echo "✓ STRAŻ ŻYWA: odnawia CAŁĄ tkankę (nie tylko pierwszy plik), nie tyka kanonu, nie wciąga obcych, odmawia na konflikcie, nie widzi końców linii, widzi treść, pilnuje składu listy TKANKA."
    exit 0
  fi
  echo "✗ STRAŻ MARTWA: H_TK=$H_TK H_MU=$H_MU H_KA=$H_KA OBCY=$ILE_OBCY RC_MARK=$RC_MARK LF=$H_LF CRLF=$H_CRLF ZM=$H_ZM B1=$H_B1 B2=$H_B2 STRAZ=$B_STRAZ TKANKA=$ILE_TKANKA"
  exit 1
fi
