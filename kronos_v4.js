...
function tzOff(y,m,d){const mar=lastSundayUTC(y,2),oct=lastSundayUTC(y,9);const t=Date.UTC(y,m-1,d);return(t>=Date.UTC(y,2,mar,1)&&t<Date.UTC(y,9,oct,1))?2:1;}
function localDate(y,m,d,hLocal){const off=tzOff(y,m,d);return new Date(Date.UTC(y,m-1,d,hLocal-off));}
function fmt(date){const ms=date.getTime();const probe=new Date(ms);const off=tzOff(probe.getUTCFullYear(),probe.getUTCMonth()+1,probe.getUTCDate());const l=new Date(ms+off*3600000);return `${String(l.getUTCDate()).padStart(2,'0')}.${String(l.getUTCMonth()+1).padStart(2,'0')} ${String(l.getUTCHours()).padStart(2,'0')}:${String(l.getUTCMinutes()).padStart(2,'0')} ${off===2?'CEST':'CET'}`;}
...
function analyze(y,m,d,hLocal){
  const date=localDate(y,m,d,hLocal||12);
  ...
  return {date,s,c,ph,decl,sadzenie,asp,next,off:tzOff(y,m,d)};
}
...
if(require.main===module){
  ...
  console.log(`\n📅 ${d}.${String(m).padStart(2,'0')}.${y} ${String(h||12).padStart(2,'0')}:00 ${r.off===2?'CEST':'CET'}`);
