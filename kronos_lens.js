...
function _lastSunPL(y,m0){const x=new Date(Date.UTC(y,m0+1,0));return x.getUTCDate()-x.getUTCDay();}
function tzOffPL(ms){const y=new Date(ms).getUTCFullYear();const mar=_lastSunPL(y,2),oct=_lastSunPL(y,9);return(ms>=Date.UTC(y,2,mar,1)&&ms<Date.UTC(y,9,oct,1))?2:1;}
