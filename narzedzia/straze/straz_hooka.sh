...
if [ "${1:-}" = "--test" ]; then
  ...
  for _b in niepelny-ladunek-przeszedl artefakt-interpolacji-przeszedl PAT-przeszedl-przez-hook niepoprawny-py-przeszedl marker-konfliktu-przeszedl; do
...
# (−/⊙/⊙) #37 MARKERY KONFLIKTU: marker ZAKOTWICZONY na początku wiersza MUSI zablokować;
# ten sam wzorzec w bloku ``` i w inline-code MUSI przejść — księgi cytują go w prozie
# (`grep -c "<<<<<<<"` stoi w PROMPCIE przeglądu), a alarm szerszy niż sygnał uczy nie
# patrzeć (#56, lekcja ZAMKA #12). Fikstury składane printf-em, nie literałem w tym pliku.
_M7='<<<<<<<'; _R7='======='; _P7='>>>>>>>'
printf '%s HEAD\na\n%s\nb\n%s galaz\n' "$_M7" "$_R7" "$_P7" > konflikt.md
git add -A
git commit -qm "marker konfliktu" >/dev/null 2>&1 && zle="$zle marker-konfliktu-przeszedl"
rm -f konflikt.md && git add -A && git commit -qm sprzatanie3 >/dev/null 2>&1
printf 'opis wzorca\n```\n%s HEAD\n%s\n%s b\n```\n' "$_M7" "$_R7" "$_P7" > cytat.md
git add -A
git commit -qm "marker w bloku kodu" >/dev/null 2>&1 || zle="$zle marker-w-bloku-kodu-blokuje"
printf 'zdanie o `%s` w prozie\n' "$_M7" > proza.md
git add -A
git commit -qm "marker inline" >/dev/null 2>&1 || zle="$zle marker-inline-blokuje"

# (−/+) #31 AMEND NA PUSHNIĘTYM/NIEPUSHNIĘTYM (19.09.2026): wymaga PRAWDZIWEGO origin
# (merge-base --is-ancestor pyta o gałąź śledzącą) — zamek na /proc, więc tor ma sens
# tylko tam, gdzie /proc istnieje (Linux); poza nim ZAMEK SAM SIĘ WYŁĄCZA (dokumentowane
# w hooku), więc oba przypadki niżej muszą PRZEJŚĆ bez blokady na platformie bez /proc —
# tor to rozróżnia (⊙), nie udaje pewności, której nie ma.
if [ -r "/proc/self/cmdline" ]; then
  ORIG="$(mktemp -d)"; git init -q --bare "$ORIG"
  git remote add origin "$ORIG" 2>/dev/null || git remote set-url origin "$ORIG"
  BR="$(git symbolic-ref --short HEAD)"
  git push -q origin "$BR:$BR" >/dev/null 2>&1
  git branch -q --set-upstream-to="origin/$BR" "$BR" >/dev/null 2>&1

  # (−) HEAD JUŻ NA origin: amend MUSI oblać
  echo "pushniete" >> plik.md && git add plik.md
  git commit --amend -qm "amend na pushnietym" >/dev/null 2>&1 \
    && zle="$zle amend-na-pushnietym-przeszedl"

  # (−) TEN SAM PRZYPADEK, ALE Z PUSTYM STAGE (19.09.2026, zmierzone na żywym repo):
  # `git commit --amend -m` bez `git add` to NAJCZĘSTSZA forma amendu (poprawka samej
  # wiadomości). Pierwotny tor robił `git add` przed amendem i dlatego świecił zielono
  # nad zamkiem, który stał PO bramce pustego stage w hooku i nigdy się nie odpalał.
  # Bez tego przypadku tor mierzy węższą rzecz, niż obiecuje jego nazwa.
  git commit --amend -qm "amend pusty stage na pushnietym" >/dev/null 2>&1 \
    && zle="$zle amend-pusty-stage-przeszedl"

  # (+) świeży, NIEpushnięty commit: amend MUSI przejść
  echo "lokalny" > swiezy.md && git add swiezy.md
  git commit -qm "swiezy niepushniety" >/dev/null 2>&1 || zle="$zle swiezy-commit-zablokowany"
  echo "lokalny2" >> swiezy.md && git add swiezy.md
  git commit --amend -qm "amend na niepushnietym" >/dev/null 2>&1 \
    || zle="$zle amend-na-niepushnietym-zablokowany"
  rm -rf "$ORIG"
fi

if [ -z "$zle" ]; then
  echo "  ✓ HOOK ŻYWY: blokuje niepełny ładunek (#35), nieświeży odcisk (#67), PAT, niesparsowany .py (⓬)"
  echo "    i amend na pushniętym (#31); przepuszcza commit kompletny, plik nieśledzony, polską treść"
  echo "    w potrójnych cudzysłowach i amend na niepushniętym (bez alarmu szerszego niż sygnał)."
  exit 0
fi
