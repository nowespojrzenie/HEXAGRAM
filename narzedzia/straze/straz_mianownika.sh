...
if [ "${1:-}" = "--test" ]; then
  ...
  zle=""
  # ── FIKSTURA KOLEJKI (13.09.2026, naprawa) ──────────────────────────────────
  # POWÓD: tor czytał ŻYWY `kanon/ksiegi/KOLEJKA_M.md` zamiast własnej fikstury — świadek
  # opierał się na mechanizmie, który mierzy (#49). Wiersz #71 istnieje na `prywatna`,
  # ale na `main` KOLEJKA_M.md jest CELOWO zalążkowana (prywatna tkanka nie podróżuje,
  # PRAWO ZALĄŻKA) — tor obcy zależał od treści, którą inne prawo świadomie wycina.
  # Fikstura czyni tor hermetycznym: prawda o legalności statusu nie może zależeć od tego,
  # która gałąź jest wyewidencjonowana. Zmierzone 13.09: bateria oblewała na `main` (1),
  # zdawała na `prywatna` (0) — ten sam kod, dwa różne werdykty z powodu żywego pliku.
  T_KOL="$(mktemp -d)"
  printf '| 71 | test | prawo | ŻYWE REPO | M |\n' > "$T_KOL/KOLEJKA_M.md"
  export KOLEJKA="$T_KOL/KOLEJKA_M.md"
  ...
  # to połowa toru (#75). #71 ma wiersz w FIKSTURZE; numer 999 nie ma i mieć nie będzie.
