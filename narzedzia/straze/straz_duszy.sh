...
WYJATKI="0_MELDUNEK.txt README.md START_TU.md kanon/ksiegi/6_ROLA_ARCHITEKTA.md kanon/prawa/TEST_DUSZY.md"
...
analiza() {
  ( cd "$1" || { echo "  x katalog nieosiagalny: $1"; echo "9 0"; return; }
    ...
    # ── ZALĄŻEK TO BRAK DANYCH, NIE KŁAMSTWO MAPY (13.09.2026) ─────────────────────────
    # Ta sama klasa i ta sama naprawa co w `lint_bledy.js` tego samego dnia. ZMIERZONE na
    # odlewie: DUSZA.md stoi na liście ZALAZKOWANE, a jej zalążek wymienia `SPIS_CIALA.md`,
    # który jest GENEROWANY i w odlewie nie jedzie. Straż orzekała „MAPA KLAMIE" na pliku,
    # który JAWNIE deklaruje, że jest pustką czekającą na narodziny — czyli mierzyła brak
    # danych jak wadę (#18, #82). Skutek: świadek martwy na `main`, `publikuj.sh` odmawiał.
    # Zalążek nie ma mapy, bo nie ma jeszcze ciała — to norma odlewu, nie usterka.
    # Wycinanie backticków WIERSZ PO WIERSZU (`sed` liniowo), bo znacznik stoi w bloku ```
    # — wieloliniowy wzorzec zjadłby cały blok razem z deklaracją (blizna z tej samej doby).
    if sed 's/`[^`]*`//g' DUSZA.md 2>/dev/null | grep -q "⟠ ZALĄŻEK"; then
      echo "  i DUSZA jest ZALAZKIEM (⟠) — mapa ciala jeszcze nie istnieje, to NIE jest wada."
      echo "    Po narodzinach (bash narodziny.sh) i wygenerowaniu spisu"
      echo "    (node narzedzia/przyrzady/spis_ciala.js) straz zacznie mierzyc parytet mapy."
      echo "0 0"; return
    fi
...
if [ "$TEST" -eq 1 ]; then
  ...
  # ⟠ ZALĄŻEK — DUSZA jeszcze nie urodzona: mapa nie istnieje, bo nie ma ciała.
  # Fikstura wiernie odtwarza kształt z `zalazkuj.sh`: znacznik stoi w bloku ```,
  # więc pilnuje też tego, że wycinanie backticków idzie WIERSZAMI, nie przez cały tekst.
  mkdir -p "$BAZA/zalazek"
  printf '# DUSZA — zalazek\n\n```\n⟠ ZALĄŻEK — tkanka nowego twórcy\nTen plik nie podrozuje.\n```\n\nmapa: `WIDMO.md`, cialo: `kanon/ksiegi/SPIS_CIALA.md`\n' > "$BAZA/zalazek/DUSZA.md"
  # ⊗ znacznik w BACKTICKACH to mowa O zalążku, nie deklaracja (28.08) — MUSI oblać
  mkdir -p "$BAZA/cytat"
  printf '# DUSZA\n\nPliki oznaczone `⟠ ZALĄŻEK` to ramy bez tresci.\n\nmapa: `WIDMO.md`, cialo: `kanon/ksiegi/SPIS_CIALA.md`\n' > "$BAZA/cytat/DUSZA.md"
  ...
  echo "── TEST + ZALĄŻEK (mapa niemierzalna, MUSI przejść i NAZWAĆ przyczynę) ──"
  out_zal="$(raport "$BAZA/zalazek")"; rc_zal=$?
  echo "$out_zal"
  case "$out_zal" in *"ZALAZKIEM"*) ;; *) rc_zal=99 ;; esac
  echo "── TEST ⊗ (znacznik ⟠ w BACKTICKACH to nie zalążek, MUSI oblać) ──"
  raport "$BAZA/cytat" >/dev/null; rc_cyt=$?
  echo "   rc(cytat)=$rc_cyt"
  rm -rf "$BAZA"
  echo
  if [ "$rc_plus" -eq 0 ] && [ "$rc_klam" -eq 1 ] && [ "$rc_niep" -eq 0 ] && [ "$rc_sc" -eq 1 ] \
     && [ "$rc_wyj" -eq 0 ] && [ "$rc_licz" -eq 0 ] && [ "$rc_bez" -eq 1 ] \
     && [ "$rc_zal" -eq 0 ] && [ "$rc_cyt" -eq 1 ]; then
  ...
  echo "✗ STRAŻ MARTWA: rc_plus=$rc_plus rc_klam=$rc_klam rc_niep=$rc_niep rc_scisle=$rc_sc rc_wyj=$rc_wyj rc_liczba=$rc_licz rc_bezspisu=$rc_bez rc_zalazek=$rc_zal rc_cytat=$rc_cyt"
  exit 1
fi
