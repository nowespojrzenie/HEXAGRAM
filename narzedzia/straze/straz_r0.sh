...
wycinek_prarodziny() {
  # ROZDZIELENIE (18.09.2026, słowo twórcy „rozdzielaj i domykaj" — po ważeniu za/przeciw).
  # PRZED tą zmianą `wycinek_bledy` liczyła DWIE różne wielkości pod jedną nazwą:
  # prozę „sześć praw-matek" (statyczna, 1,9–2,0 kB od lipca) i INDEKS KANONICZNY
  # (tabela wszystkich blizn, rośnie ~1 wiersz na każdą nową — zmierzone: 10,9 kB
  # 26.08 → 34,7 kB 18.09, POTROIŁA SIĘ, podczas gdy reszta R0 w tym samym czasie
  # oddychała 24,7–34,5 kB i WRACAŁA). Próg 36000 ma rodowód na wielkości, która
  # oddycha — do wielkości rosnącej monotonicznie nie pasuje z definicji: alarm,
  # który nie może zgasnąć, przestaje być alarmem (rodzina #38).
  # Granica: PRARODZINY kończy się tam, gdzie zaczyna INDEKS — nie w domysłach,
  # w nagłówku `## ▣ INDEKS` (sprawdzone 18.09: jedno wystąpienie w całej księdze,
  # na początku linii, żaden cytat go nie naśladuje — bez pułapki #50).
  awk '/^## ⬡ PRARODZINY/{f=1} /^## ▣ INDEKS/{f=0} f' "${1:-kanon/ksiegi/BLEDY.md}" 2>/dev/null \
    | filtr_satelitow | wc -c
}

wycinek_indeks() {
  # Odwrotność wycinek_prarodziny: WYŁĄCZNIE tabela, od `## ▣ INDEKS` do pierwszej
  # blizny numerowanej. Rośnie z natury — to jest nawigacja (grep, nie lektura),
  # nie proza do przeczytania przy wstaniu. Osobna liczba, własne, luźniejsze
  # kryterium niżej (komunikat), nie PROG R0.
  awk '/^## ▣ INDEKS/{f=1} /^## #[0-9]+/{f=0} f' "${1:-kanon/ksiegi/BLEDY.md}" 2>/dev/null \
    | filtr_satelitow | wc -c
}
...
zmierz_r0() {
  ...
  # OD 18.09: liczy wycinek_prarodziny (proza), NIE wycinek_indeks (tabela) — patrz
  # rodowód rozdziału przy definicji wycinek_prarodziny.
  local t=0 f
  for f in DUSZA.md 0_SNAPSHOT_watek.md 0_WYWOLANIA.md; do
    [ -e "$f" ] && t=$((t + $(bez_satelitow "$f")))
  done
  echo $((t + $(wycinek_prarodziny kanon/ksiegi/BLEDY.md)))
}

zmierz_indeks() {
  # Osobna wielkość (18.09.2026) — nawigacja, nie proza. Rośnie z natury o każdą
  # nową bliznę; komunikat ją melduje, ale PROG R0 jej nie mierzy.
  wycinek_indeks kanon/ksiegi/BLEDY.md
}

komunikat() {
  local r0="$1" prog="$2" sat="${3:-}" idx="${4:-}"
  if [ -n "$sat" ]; then
  ...
  if [ -n "$idx" ]; then
    echo "   ⓘ INDEKS KANONICZNY = ${idx} B — rośnie z natury (≈1 wiersz/blizna), poza progiem R0"
    echo "     (rozdzielone 18.09: to nawigacja — grep, nie lektura przy wstaniu. Kompostuje się"
    echo "      przy przeglądzie struktury, nie na alarm R0. Rodowód: TASKI.md, wątek »R0 sprawca«.)"
  fi
  if [ "$r0" -gt "$prog" ]; then
...
if [ "${1:-}" = "--test" ]; then
  ...
  # (①) PROZA nie połyka ani INDEKSU, ani korpusu — nagłówek + "AAAA\n" (23 B, zmierzone
  #      na fiksturze: własny nagłówek wchodzi do wycinka, tak jak w oryginalnej definicji).
  W1="$(wycinek_prarodziny "$TR/BLEDY.md")"
  [ "$W1" -eq 23 ] || zle="$zle proza-nie-jest-czysta($W1, ma być 23)"
  # (①b) INDEKS nie połyka ani PROZY, ani korpusu — własny nagłówek + "BBBB\n" (19 B).
  #      LUSTRO testu wyżej — bez tego byłoby wiadomo, że PROZA jest czysta,
  #      ale nie wiadomo, czy nadmiar wylądował w INDEKSIE, czy zniknął po cichu.
  I1="$(wycinek_indeks "$TR/BLEDY.md")"
  [ "$I1" -eq 19 ] || zle="$zle indeks-nie-jest-czysty($I1, ma być 19)"
  # (①c) BRAK LUKI I BRAK NAKŁADKI: suma dwóch nowych wycinków musi równać się temu,
  #      co dał BY stary, niepodzielony zakres PRARODZINY→pierwsza blizna — inaczej
  #      rozdział coś zgubił albo policzył podwójnie w granicy.
  CALOSC_STARA=$(awk '/^## ⬡ PRARODZINY/{f=1} /^## #1 /{f=0} f' "$TR/BLEDY.md" | wc -c)
  [ $((W1 + I1)) -eq "$CALOSC_STARA" ] || zle="$zle luka-lub-nakladka-w-granicy($W1+$I1≠$CALOSC_STARA)"
  # (②) przemianowanie TYTUŁU nie rusza żadnego pomiaru — kotwica po wzorcu, nie po napisie
  sed -i 's/## #1 (❶) RDZEN/## #1 (❶) ZUPELNIE INNY TYTUL/' "$TR/BLEDY.md"
  W2="$(wycinek_prarodziny "$TR/BLEDY.md")"; I2="$(wycinek_indeks "$TR/BLEDY.md")"
  [ "$W2" -eq "$W1" ] || zle="$zle proza-zależna-od-tytułu($W1→$W2)"
  [ "$I2" -eq "$I1" ] || zle="$zle indeks-zależny-od-tytułu($I1→$I2)"
  # (②b) CYTAT wzorca PRARODZINY w korpusie NIE MOŻE włączać flagi (21.08). Fikstura
  #      odróżnialna: ta sama treść raz jako nagłówek (liczona), raz w środku zdania.
  printf 'tekst o bledzie: kotwica `## ⬡ PRARODZINY` byla zla\n' >> "$TR/BLEDY.md"
  printf 'Y%%.0s' $(seq 1 5000) >> "$TR/BLEDY.md"
  W3="$(wycinek_prarodziny "$TR/BLEDY.md")"
  [ "$W3" -eq "$W2" ] || zle="$zle cytat-prarodzin-wlacza-flage($W2→$W3)"
  # (②c) LUSTRO na drugiej granicy — cytat `## ▣ INDEKS` w korpusie nie może włączać
  #      flagi INDEKSU. Ten sam kształt wady, nowa krawędź (18.09, dopisana przy rozdziale,
  #      zanim zdążyła się zdarzyć na żywym repo — nie po fakcie).
  printf 'tekst o bledzie: naglowek `## ▣ INDEKS` bywa cytowany\n' >> "$TR/BLEDY.md"
  printf 'Z%%.0s' $(seq 1 5000) >> "$TR/BLEDY.md"
  I3="$(wycinek_indeks "$TR/BLEDY.md")"
  [ "$I3" -eq "$I2" ] || zle="$zle cytat-indeksu-wlacza-flage($I2→$I3)"
  # (③) na ŻYWEJ księdze: PROZA mała i STABILNA (rodowód: 1,9–2,0 kB od lipca),
  #     INDEKS duży i to JEST W PORZĄDKU — to jest właśnie sens rozdziału (18.09).
  #     Próg 5000 ma margines: proza rosła 1954→1954 B przez trzy tygodnie, 5000
  #     to >2,5× tego stanu, nie granica przypadkowa.
  ZYWA_P=$(wycinek_prarodziny kanon/ksiegi/BLEDY.md)
  [ "$ZYWA_P" -lt 5000 ] || zle="$zle proza-urosla-ponad-margines($ZYWA_P)"
  ZYWY_I=$(wycinek_indeks kanon/ksiegi/BLEDY.md)
  [ "$ZYWY_I" -gt "$ZYWA_P" ] || zle="$zle indeks-nie-jest-wiekszy-od-prozy($ZYWY_I≤$ZYWA_P)"
  # (③b) DOWÓD SENSU ROZDZIAŁU: R0 (liczony przez zmierz_r0, czyli PRAWDZIWA formuła
  #      użyta w komunikacie) NIE zawiera indeksu — musi być mniejszy niż R0+INDEKS.
  R0_ZYWY=$(zmierz_r0)
  [ "$R0_ZYWY" -lt $((R0_ZYWY + ZYWY_I)) ] || zle="$zle r0-nie-mniejszy-od-calosci"
  [ "$R0_ZYWY" -lt 36000 ] || zle="$zle r0-nadal-nad-progiem-po-rozdziale($R0_ZYWY)"
  ...
  rm -rf "$TS"
  if [ -z "$zle" ]; then
    echo "  ✓ STRAŻ ŻYWA — 25 torów: świeci na przekroczeniu, milczy pod progiem,"
    echo "    równość przepuszcza, formuła mierzy, miękkość zaryglowana torem,"
    echo "    DRUGA LICZBA łapie przenosiny (26.08), a od 18.09 TRZECIA granica —"
    echo "    PROZA i INDEKS rozdzielone, każda ma własny tor na cytat i tytuł,"
    echo "    suma obu zgadza się co do bajtu ze starym, niepodzielonym zakresem,"
    echo "    a R0 na żywej księdze mierzalnie mniejsze niż R0+INDEKS i pod progiem."
    exit 0
  fi
...
komunikat "$(zmierz_r0)" "$PROG" "$(zmierz_satelity)" "$(zmierz_indeks)" || true
exit 0
