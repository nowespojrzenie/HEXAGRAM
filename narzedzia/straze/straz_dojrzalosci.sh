...
# LISTA WYŁĄCZEŃ ODLEWU — czytana WYŁĄCZNIE po to, by JEJ NIE LICZYĆ jako dowodu użycia
# (patrz `wolany()`). Parametr, nie literał, żeby tor mógł podstawić własną atrapę.
ODLEW="${ODLEW:-narzedzia/straze/straz_odlewu.sh}"
...
wolany() {
  ...
  # LISTA WYŁĄCZEŃ NIE DOWODZI UŻYCIA (20.09.2026, zmierzone — klasa #59/#49). `$ODLEW`
  # trzyma wpisy `nazwa|powód`, więc SAMA NAZWA przyrządu stoi w jego ciele jako literał.
  # Ta warstwa pyta gołym `grep -l`, więc każdy wpis na liście wyłączeń fałszywie dowodził
  # własnego użycia — dokładnie ten sam argument kołowy, dla którego wyłączony jest `$PUBLIKUJ`
  # („jest na liście" ≠ „jest używany"). Zmierzone przed naprawą: `pomiar_ksiag.sh` zdawał
  # kryterium 5 WYŁĄCZNIE dzięki zdaniu tłumaczącemu, czemu go nie ma w codziennej pracy.
  # Dla pozostałych sześciu wpisów nie szkodziło, bo mają wzmianki niezależne — przypadek,
  # nie konstrukcja. Zmierzony zasięg naprawy: jeden przyrząd traci fałszywy pozytyw, zero
  # utraconych prawdziwych (`destyluj_ksiege.js` łapie wcześniejsza warstwa wywołań).
  [ "$(grep -l "$(basename "$1")" *.sh *.js package.json narzedzia/*/*.sh narzedzia/*/*.js .githooks/pre-commit 2>/dev/null \
       ...
       | grep -vx "$(basename "$PUBLIKUJ")" | grep -vx "$(basename "${ODLEW:-narzedzia/straze/straz_odlewu.sh}")" \
       | grep -vx "$JA" | wc -l)" -gt 0 ]
}
...
if [ "${1:-}" = "--test" ]; then
  ...
  run() { ( cd "$T"; BATERIA=bat.sh MUTACJE=mut.txt HASHE=hash.txt MOSTY=mosty.md PUBLIKUJ=pub.sh \
            ODLEW=odlew.sh PULPIT=pulpit.md \
  ...
  # (−) LISTA WYŁĄCZEŃ NIE DOWODZI UŻYCIA (20.09.2026 — klasa #59/#49, argument kołowy).
  #     Sierota BEZ wołającego, ale ZE wpisem `nazwa|powód` w liście wyłączeń odlewu.
  #     Przed naprawą ten przypadek PRZECHODZIŁ: sama nazwa w zdaniu tłumaczącym, czemu
  #     przyrządu nie ma w codziennej pracy, zasilała `grep -l` jako dowód, że jest używany.
  rm -f "$T/wolajacy.sh"
  printf 'dojrzaly.sh|raport na zadanie, nie codzienna praca\n' > "$T/odlew.sh"
  spr "wpis na liscie WYLACZEN nie liczy sie jako wolanie (OBLEWA)" "$(run)" "1"

  # (+) ODRÓŻNIALNOŚĆ: ten sam brak wołającego, ale komenda stoi w PULPICIE — ręka jest
  #     wołającym (prawo 29.08). Bez tej pary „(−) oblewa" byłoby nieodróżnialne od
  #     „każda sierota oblewa zawsze", a naprawa byłaby przetestowana połowicznie (#75).
  printf '# pulpit\n4d. cos tam -> `bash dojrzaly.sh` (raport)\n' > "$T/pulpit.md"
  spr "komenda w PULPICIE liczy sie jako wolanie (przechodzi)" "$(run)" "0"
  rm -f "$T/pulpit.md" "$T/odlew.sh"
  printf '#!/bin/sh\nbash dojrzaly.sh\n' > "$T/wolajacy.sh"
