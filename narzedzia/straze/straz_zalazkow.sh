...
zalazek_ok() {  # $1 = ścieżka
  ...
  # ── FORMAT DAT: DEKLARACJA (U7, 21.09.2026) ──────────────────────────────────────────────
  # Straż rozpoznaje DWA formaty i tylko je: DD.MM.RRRR oraz RRRR-MM-DD, lata 19xx i 20xx.
  # Do 21.09 znała jeden (`DD.MM.20xx`): „Urodzony 1984-03-12 w Poznaniu" przechodziło jako
  # czyste (mutant Fable, rzut 013) — a data urodzenia jest właśnie tym, co zalążek ma nie nieść.
  # NIE łapie zapisów słownych („12 marca 1984"), ukośnikowych ani samego roku — to granica
  # przyrządu, nie gwarancja: realną zaporą PII jest biała lista `publikuj.sh`, ta straż jest
  # drugim, węższym sitem (#56: alarm szerszy niż sygnał uczy nie patrzeć).
  grep -qE '\b[0-3][0-9]\.[01][0-9]\.(19|20)[0-9][0-9]\b' "$_p" 2>/dev/null && _wady="${_wady} DATA-z-drogi-pierwotnej"
  grep -qE '\b(19|20)[0-9][0-9]-[01][0-9]-[0-3][0-9]\b' "$_p" 2>/dev/null && _wady="${_wady} DATA-ISO-z-drogi-pierwotnej"
  [ -z "$_wady" ] && return 0
  echo "$_wady"; return 1
}
...
if [ "$TRYB" = "--test" ]; then
  ...
  # − KAŻDY WARUNEK MA FIKSTURĘ Z JEDNĄ WADĄ (U7, 21.09.2026). Mutanty Fable (rzut 013):
  # wyłączenie wymogu znacznika ⟠ i wyłączenie zdania „nie podróżuje" PRZEŻYŁY, bo `goly.md`
  # i `kadlub.md` łamią po 2–3 warunki naraz — zdjęcie jednego z nich niczego nie zmieniało w rc.
  # Tu każda fikstura ma dokładnie jedną wadę, a tor sprawdza NAZWĘ wady, nie sam rc: mutacja,
  # która przepnie warunek na inną przyczynę, też oblewa (#49: oblewa z jednego powodu).
  wada_dokl() {  # $1 = plik · $2 = oczekiwana lista wad (jak drukuje zalazek_ok, bez wiodącej spacji)
    local w; w="$(WZORZEC="$T/wzor.txt" zalazek_ok "$1" 2>/dev/null)"; [ "${w# }" = "$2" ]
  }
  printf 'Ten plik nie podróżuje.\nSkoro go potrzebujesz — jesteś twórcą.\n' > "$T/bez_znacznika.md"
  wada_dokl "$T/bez_znacznika.md" "brak-znacznika-⟠" && R_BZN=0 || R_BZN=1
  printf '⟠ ZALĄŻEK — tkanka nowego twórcy\nSkoro go potrzebujesz — jesteś twórcą.\n' > "$T/bez_niepodrozowania.md"
  wada_dokl "$T/bez_niepodrozowania.md" "brak-zdania-o-niepodróżowaniu" && R_BNP=0 || R_BNP=1
  wada_dokl "$T/bezautorstwa.md" "brak-oddania-autorstwa" && R_AUT2=0 || R_AUT2=1
  # − FORMATY DAT: DD.MM.RRRR dla 19xx (data urodzenia) i ISO RRRR-MM-DD — każdy osobno.
  printf "$NAGL" > "$T/data19.md"; printf 'nadane 14.07.1984\n' >> "$T/data19.md"
  wada_dokl "$T/data19.md" "DATA-z-drogi-pierwotnej" && R_D19=0 || R_D19=1
  printf "$NAGL" > "$T/iso.md"; printf 'Urodzony 1984-03-12 w Poznaniu\n' >> "$T/iso.md"
  wada_dokl "$T/iso.md" "DATA-ISO-z-drogi-pierwotnej" && R_ISO=0 || R_ISO=1
  # ⊘ para dla ISO (#75): liczba przypominająca datę, ale nią niebędąca (wersja, przedział) — cisza.
  printf "$NAGL" > "$T/nieiso.md"; printf 'wersja 1.7.1, zakres 2026-2027, tabela 12-03\n' >> "$T/nieiso.md"
  WZORZEC="$T/wzor.txt" zalazek_ok "$T/nieiso.md" >/dev/null 2>&1 && R_NIS=0 || R_NIS=1
  ...
  echo "── TEST − (JEDNA wada: tylko brak znacznika ⟠):        rc=$R_BZN (oczekiwane 0 = wada nazwana dokładnie)"
  echo "── TEST − (JEDNA wada: tylko brak „nie podróżuje\"):    rc=$R_BNP (oczekiwane 0 = wada nazwana dokładnie)"
  echo "── TEST − (JEDNA wada: tylko brak oddania autorstwa):  rc=$R_AUT2 (oczekiwane 0 = wada nazwana dokładnie)"
  echo "── TEST − (data DD.MM.RRRR z 19xx):                    rc=$R_D19 (oczekiwane 0 = wykryta)"
  echo "── TEST − (data ISO RRRR-MM-DD):                       rc=$R_ISO (oczekiwane 0 = wykryta)"
  echo "── TEST ⊘ (wersja/zakres/tabela nie są datą):          rc=$R_NIS (oczekiwane 0 — cisza)"
  if [ "$R_BZN" -eq 0 ] && [ "$R_BNP" -eq 0 ] && [ "$R_AUT2" -eq 0 ] && [ "$R_D19" -eq 0 ] \
     && [ "$R_ISO" -eq 0 ] && [ "$R_NIS" -eq 0 ] \
     && [ "$R_OK" -eq 0 ] && [ "$R_GOL" -eq 1 ] && [ "$R_IMI" -eq 1 ] \
