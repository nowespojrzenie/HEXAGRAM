...
indeks() {
  local WY="${ZNIWO_INDEKS:-kanon/ksiegi/DESTYLATY_indeks.md}"
  {
    ...
    echo
    # ZNACZNIK ZIARNO STOI TU, NIE W PLIKU WYJŚCIOWYM (20.09.2026). Wstawiony ręką do
    # `DESTYLATY_indeks.md` przeżył dokładnie jeden bieg tego generatora — nadpis go zdjął,
    # a plik wrócił do stanu „ani jednego znacznika", przez który `zalazkuj.sh` odmawia
    # i WSTRZYMUJE ODLEW (zmierzone tego samego dnia, kod 7). Deklaracja, co jest formą,
    # a co treścią, musi żyć w tym, co generuje — inaczej jest kasowana przez własne
    # narzędzie po cichu. Rodzina #13 (cicha podmiana) i #55 (obietnica bez czytnika).
    echo "<!-- ⟠ ZIARNO -->"
