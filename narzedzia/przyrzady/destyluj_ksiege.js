...
// ZAKRES DESTYLATU (N1+/D, 21.09.2026): dla BLEDY nota niesie linię `zakres destylatu: #1–#NNN`,
// gdzie NNN = najwyższy numer INDEKSU w chwili odlewu — JEDNO ŹRÓDŁO granicy pisane przez generator,
// nigdy przez odbiorcę. `lint_bledy.js` czyta ją i dla #N ≤ NNN traktuje „indeks bez korpusu" jako
// celową kratę destylatu (ⓘ), a dla #N > NNN (własne wpisy odbiorcy) stosuje prawo #22 w całości.
// MOSTY zakresu nie dostają: ich tabela nie ma numeracji, której lint musiałby pilnować.
const nota = (zakres) => [
  '',
  '```',
  ...
  ...(zakres ? ['zakres destylatu: #1–#' + zakres] : []),
  '```',
  ''
].join('\n');
...
function destylujBledy(zrodlo) {
  ...
  const zMech = new Set();
  let maxIdx = 0;   // najwyższy numer wiersza INDEKSU — granica zakresu destylatu
  ...
  if (i >= 0) out.splice(i + 1, 0, nota(maxIdx));
  return { tekst: out.join('\n'), zostalo: zMech.size, zakres: maxIdx };
}
...
function destylujMosty(zrodlo, nazwy) {
  ...
  const i = out.findIndex(l => /^# /.test(l));
  if (i >= 0) out.splice(i + 1, 0, nota(0));
  return { tekst: out.join('\n'), zostalo, zdjete };
}
...
if (process.argv[2] === '--test') {
  ...
  // (N1+/D) zakres pisze GENERATOR z indeksu; odbiorca go nie zna ani nie edytuje.
  spr('BLEDY: zakres destylatu = najwyzszy numer indeksu', /^zakres destylatu: #1–#2$/m.test(rb.tekst), true);
  spr('BLEDY: zakres zwracany jako liczba', rb.zakres, 2);
  ...
  spr('MOSTY: nota BEZ zakresu (tabela bez numeracji)', /zakres destylatu/.test(rm.tekst), false);
