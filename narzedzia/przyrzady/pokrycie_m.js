...
function liniaPraw() {
  ...
  return `   PRAWA (_GRANICA): ${g.praw} · ${g.zEgz} z egzekutorem (nazwany plik) · ${g.bez} bez`
       // NAPRAWA (18.09.2026, słowo twórcy): separator ' · ' kolidował z '·' WEWNĄTRZ
       // niektórych tytułów praw (np. „PRAWO PIĘCIU KSIĄG · routing zapisów") — 4 realne
       // pozycje czytały się jak 6, bo separator między elementami listy był nie do
       // odróżnienia od separatora wewnątrz jednego elementu. Rozjazd 4 vs 6 zmierzony
       // i rozstrzygnięty 18.09: licznik miał rację, prezentacja myliła. Numerowanie
       // zamiast dzielenia znakiem, który tytuły już zawierają.
       + (g.bez ? `\n      bez nazwanego egzekutora: ` + g.bezNazwy.map((n, i) => `${i + 1}) ${n}`).join('  ') : '');
}
