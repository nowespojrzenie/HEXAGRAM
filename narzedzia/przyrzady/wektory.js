...
function test() {
  ...
  // NAPRAWA 13.09.2026: asercja czytała `path.join(ROOT, 'wektory.js')` — ścieżkę prawdziwą
  // TYLKO dopóki przyrząd leżał w korzeniu. Po PRZENOSINACH 4/4 (9844666, 29.08) plik mieszka
  // w `narzedzia/przyrzady/`, więc `czytaj` dostawał nieistniejącą ścieżkę, zwracał '' i T9b
  // oblewał — a wraz z nim CAŁY tor (rc=1). Trzy pozycje rejestru mutacji, których świadkiem
  // jest `wektory.js --test`, meldowały odtąd ZŁAPANĄ niezależnie od mutacji: pokrycie POZORNE
  // od 29.08. Ten sam commit przestawiał cztery przyrządy na nowy korzeń — tę jedną asercję
  // pominął, bo `ROOT + własna nazwa` wyglądało wtedy jak „ten plik".
  // `__filename` nie zgaduje lokalizacji — JEST nią, więc następne przenosiny jej nie złamią.
  t('T9b istniejący plik nadal jest CZYTANY (odróżnialność)',
    czytaj(__filename).length > 0);
