...
const lastSundayUTC=TZ.lastSundayUTC;
// tzOffset zachowane w API dla zgodności — ziarno teraz INSTANTOWE, nie dobowe
function tzOffset(year, month, day, hour){ return TZ.offLocal(year, month, day, TZ.godzWejscia(hour)); }
...
function localToJD(y,m,d,hLocal){ return 2440587.5 + TZ.localToUTC(y,m,d,TZ.godzWejscia(hLocal))/86400000; }
function jdToLocal(j){ const ms=(j-2440587.5)*86400*1000; return new Date(ms+TZ.offAt(ms)*3600*1000); }
function fmtLocal(j){ return TZ.fmt((j-2440587.5)*86400000); }  // dni[] był martwym kodem
...
function analyze(y,m,d,hLocal){
  ...
  return {j,s,c,ph,pn,ps,decl,sadzenie,next,off:TZ.offAt((j-2440587.5)*86400000)};
}
...
if(require.main===module){
  ...
  if(cmd==="day"){
    ...
    console.log(`\n📅 ${d}.${String(m).padStart(2,'0')}.${y} ${String(TZ.godzWejscia(h)).padStart(2,'0')}:00 ${tzName(r.off)}`);
