...
if(require.main===module){
  ...
  if(args[0]==='now' || args.length===0){
    ...
    date = TZ.localDate(y, m, d, TZ.godzWejscia(h));   // było: stałe -2 = CEST przez cały rok
    label = `${d}.${String(m).padStart(2,'0')}.${y} ${String(TZ.godzWejscia(h)).padStart(2,'0')}:00 ${TZ.nazwa(TZ.offAt(date))}`;
  }
