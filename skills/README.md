...
Install one, or all five. They work standalone and declare each other as siblings.
...
The prefix is a warning label, and it is meant to be one. These two are not the same sort of thing as the five above: the first is arithmetic that happens to serve astrology, the second is an interpretive lens that can never be evidence. **Read the top of each before loading it.**
...
## How the `ai-` five fit together

They are four layers of one posture — **measure, record, don't decorate, don't invent** — plus a volume control.
...
- **Format:** conform to the [Agent Skills specification](https://github.com/agentskills/agentskills). All five validate against the reference `skills-ref` implementation.
- **Sizes:** 5.8–9.1 KB each. Small enough to read before trusting.
