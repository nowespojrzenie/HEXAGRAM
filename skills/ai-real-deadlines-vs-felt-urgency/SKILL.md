...
Do **not** load this when the deadline is already known, external, and written down.
Then the question is sequencing, not urgency, and this skill only adds a step.
...
Same shape as the death criterion in `ai-preregistration-confirmation-bias` — there it kills a claim, here it kills a plan item. Don't build a second field for it; one mechanism, applied to whatever needs a closing condition, including a project's own open-thread register.
