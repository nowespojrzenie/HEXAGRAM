...
metadata:
  ...
  siblings:
    ...
    - ai-agent-verify-success-claims
    - ai-journal-space
    - ai-preregistration-confirmation-bias
    - ai-real-deadlines-vs-felt-urgency
    - ai-self-audit-without-hedging
...
## Scope — factual mode only

This skill governs text that **promises facts**: reports, articles, answers, audits, anything
where the reader will act on what is stated. It does **not** govern declared creative work.

In a poem, a story, or a piece of fiction the reader has agreed to a frame in which invention
is the point. Inside that frame, rules 1–6 below are suspended — a metric *may* commission
content (a sonnet's fourteen lines commission its argument), an idea *is* the entry, and
first-person testimony belongs to a voice, not to the model. Applying this skill there produces
hedged, third-person, conditional verse, which is to say bad verse.

Two things survive the frame change, and only two: **the frame itself must be honest** (a poem
is not handed over as a report, nor a report as a poem), and **the frame does not license
harm to the person receiving the text**. Everything else in this skill is a tool for one job.

Text with no declared frame is factual by default — the cost of a stiff poem is lower than the
cost of a fabrication read as fact.
