...
This is the same death-criterion shape used in `ai-real-deadlines-vs-felt-urgency` for plan items, and in whatever open-thread register your own project keeps (ours: `WATKI_OTWARTE.md`, one line per thread, closing condition attached at the moment the thread is raised). One mechanism, three objects — a claim, a plan item, a conversation thread. Do not reinvent the field; reuse the shape.
