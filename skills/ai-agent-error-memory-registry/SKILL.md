...
A guard that has never returned "no" is not known to work. Show it a case it **must** reject before you trust it.

This is the single most common way error-prevention machinery turns decorative: it is installed, it goes green, and the green means nothing because it has never been red. The contract form that makes a guard testable — load-bearing assumption, validation, known gap — is in `ai-agent-drift-detection` §3.
