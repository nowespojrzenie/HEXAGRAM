...
- **the instrument that silently broke.** A malformed pattern, a wrong path, an unsupported syntax — all return empty, and empty reads as *no findings*. **Test the pattern on a known hit before trusting a zero.**
...
- **Compost** — when a rule has become a mechanism, compress it to a capsule (rule, guard name, pointer) and move the full text out. Nothing is deleted; it stops competing for attention.
...
- **Signature.** Open each response with a one-line marker of which mode is active. Cheap, and drift becomes visible in the transcript rather than only in behaviour.
