...
skills/README.md skills/ai-agent-drift-detection/SKILL.md skills/ai-agent-error-memory-registry/SKILL.md skills/ai-agent-verify-success-claims/SKILL.md skills/ai-hallucination-truth-status/SKILL.md skills/ai-interview-longitudinal/SKILL.md skills/ai-journal-space/SKILL.md skills/ai-personas-not-modes/SKILL.md skills/ai-preregistration-confirmation-bias/SKILL.md skills/ai-real-deadlines-vs-felt-urgency/SKILL.md skills/ai-self-audit-without-hedging/SKILL.md skills/astro-six-element-lens/SKILL.md skills/astro-transit-window-boundaries/SKILL.md"
  ...
  if [ -n "$TRYB_LISTA" ]; then
    ...
    case "$TRYB_LISTA" in
      ...
      EKSPORT|WYJATKI|ZALAZKOWANE|NIGDY|MAIN|NARZEDZIA_MAIN|ZAKAZANE) TRYB_LISTA_POZNO="$TRYB_LISTA" ;;
      *) echo "publikuj --lista: nieznana sekcja '$TRYB_LISTA' (MD|JS|SH|INNE|WSZYSTKO|EKSPORT|WYJATKI|ZALAZKOWANE|NIGDY|MAIN|NARZEDZIA_MAIN|ZAKAZANE)" >&2; exit 2 ;;
    esac
...
ZALAZKOWANE="DUSZA.md kanon/tozsamosc/ARCHITEKT_istnienie.md kanon/ksiegi/LOG_SESJI.md kanon/ksiegi/KOLEJKA_M.md \
  ...
  NARZEDZIA_MAIN=".gitignore .gitattributes _STRAZ_wyjatki.txt _STRAZ_wzorzec.txt"
  ...
  if [ -n "${TRYB_LISTA_POZNO:-}" ]; then
    case "$TRYB_LISTA_POZNO" in
      EKSPORT) _l_drukuj "$FORMA_MD"; _l_drukuj "$FORMA_JS"; _l_drukuj "$FORMA_SH"; _l_drukuj "$FORMA_INNE"; _l_drukuj "$FORMA_ZAKAZANE_WYJATKI" ;;
      ...
      MAIN) _l_drukuj "$FORMA_MD"; _l_drukuj "$FORMA_JS"; _l_drukuj "$FORMA_SH"; _l_drukuj "$FORMA_INNE"; _l_drukuj "$FORMA_ZAKAZANE_WYJATKI"; _l_drukuj "$NARZEDZIA_MAIN" ;;
    esac
    exit 0
  fi
fi
...
if [ "${1:-}" = "--test" ]; then
  ...
  [ "$(_lp EKSPORT)" = "$( { _lp WSZYSTKO; _lp WYJATKI; } | sort -u)" ] || RC_LP=1
...
git archive HEAD -- $FORMA_MD $FORMA_JS $FORMA_SH $FORMA_INNE ${FORMA_ZAKAZANE_WYJATKI:-} | tar -x -C "$DST"
