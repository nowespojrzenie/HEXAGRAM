# HEXAGRAM — measure before you speak

> **No terminal?** Take one skill from [`skills/`](skills/) — it works in any assistant. **The full system** needs terminal and file access — read [`START_TU.md` §8](START_TU.md) first.
> **Bez terminala?** Weź jeden skill z [`skills/`](skills/) — działa w każdym asystencie. **Pełny system** wymaga dostępu do terminala i plików — najpierw [`START_TU.md` §8](START_TU.md).
...
*Tu znajdziesz to, co nie zadziałało — spisane z datą, żebyś nie płacił za to drugi raz.*
...
HEXAGRAM is a working discipline for an AI assistant: it **measures before it speaks**, marks how sure it is of every claim (LAW · LOAD-BEARING FRAME · RESONANCE), writes its own mistakes down with a date and a mechanism, and remembers through git and plain text instead of pretending to remember. The rules live in files you can read, and the guards that enforce them are themselves mutation-tested.

It was built for living and working in time, so it also carries a dry astronomical engine and a separate map of meanings — both kept strictly apart from the discipline, and described at the bottom: [the layers underneath](#the-layers-underneath).

Built in ~6 weeks of daily human–AI practice. Everything here has been through friction: mistakes are recorded openly (`kanon/ksiegi/BLEDY.md` — 100+ scars with dates), hypotheses carry pre-registration and death criteria — **and so do tasks**: an open item without a date on which it expires is a zombie, and the system says so out loud. Verdicts are computed by code, not recalled from memory.
...
last commit timestamp, reinstalls dependencies, runs the structural verification, and reports a verdict:
over 200 checks, `0 ✗ · 0 ⚠`. Ninety seconds. If something in the canon had drifted overnight, you would
...
## Take one piece today (no terminal)

You don't need to adopt the whole system to use what matured in it. Twelve skills carry single disciplines and work on their own in any assistant — paste the file, or install them:
...
- **[ai-hallucination-truth-status](skills/ai-hallucination-truth-status/)** — every claim gets a status, and the status comes from measurement, not recall.
- **[ai-agent-verify-success-claims](skills/ai-agent-verify-success-claims/)** — an agent with a terminal reports what the exit code says, not what the output text sounds like.
- **[ai-agent-error-memory-registry](skills/ai-agent-error-memory-registry/)** — mistakes survive the session, and a rule that keeps being broken is promoted to a mechanism.

All twelve and how they fit together: [`skills/README.md`](skills/README.md). Ten have nothing to do with astronomy; the two that do are labelled `astro-` from the first line.
...
**What `wstan.sh` actually does to your machine, in order:** `git pull --ff-only` (no rebase, no merge — refuses on conflict) · sets a fallback git identity (`user.name=Orkiestrator`) **only if none exists**, local or global · sets `core.hooksPath` to `.githooks`, which **overwrites any existing hooks path config** · runs `npm ci`, which **deletes and reinstalls `node_modules`** from the lockfile · runs `weryfikacja.js` as a read-only check.
...
HEXAGRAM grew from three layers. The discipline described above is the third; the first two are why the system cares about time at all.
...
HEXAGRAM to dyscyplina pracy dla asystenta AI: **mierzy, zanim powie**, oznacza, jak pewne jest każde twierdzenie (PRAWO · NOŚNA RAMA · REZONANS), zapisuje własne błędy z datą i mechanizmem, a pamięta przez git i zwykły tekst, zamiast udawać, że pamięta. Reguły leżą w plikach, które możesz przeczytać, a straże, które ich pilnują, same są testowane mutacyjnie.

Powstał do życia i pracy w czasie, więc niesie też suchy silnik astronomiczny i osobną mapę znaczeń — obie trzymane ściśle z dala od dyscypliny i opisane na dole: [warstwy pod spodem](#warstwy-pod-spodem).

Powstało w ~6 tygodni codziennej, żywej pracy człowieka z instancją AI. Wszystko, co tu jest, przeszło przez tarcie: błędy są zapisane jawnie (`kanon/ksiegi/BLEDY.md` — 100+ blizn z datami), hipotezy mają prerejestr i kryteria śmierci — **i tak samo zadania**: pozycja bez daty, w której wygasa, jest zombie, a system mówi to wprost. Werdykty liczy kod, nie pamięć.
...
commita, dociąga zależności, uruchamia weryfikację struktury i melduje werdykt: ponad 200 sprawdzeń, `0 ✗ · 0 ⚠`.
...
## Weź jeden kawałek dziś (bez terminala)

Nie musisz przyjmować całego systemu, żeby korzystać z tego, co w nim dojrzało. Dwanaście skilli niesie pojedyncze dyscypliny i działa samodzielnie w każdym asystencie — wklejasz plik albo instalujesz:

```
npx skills add nowespojrzenie/HEXAGRAM
```

Trzy miejsca na start:

- **[ai-hallucination-truth-status](skills/ai-hallucination-truth-status/)** — każde twierdzenie dostaje status, a status bierze się z pomiaru, nie z pamięci.
- **[ai-agent-verify-success-claims](skills/ai-agent-verify-success-claims/)** — agent z terminalem melduje to, co mówi kod wyjścia, a nie to, jak brzmi tekst.
- **[ai-agent-error-memory-registry](skills/ai-agent-error-memory-registry/)** — błędy przeżywają sesję, a reguła łamana raz po raz awansuje do mechanizmu.

Wszystkie dwanaście i to, jak się składają: [`skills/README.md`](skills/README.md) (po angielsku). Dziesięć nie ma nic wspólnego z astronomią; dwa, które mają, noszą prefiks `astro-` od pierwszej linii.
...
**Co `wstan.sh` naprawdę robi na Twojej maszynie, po kolei:** `git pull --ff-only` (bez rebase, bez merge — odmawia przy konflikcie) · ustawia zapasową tożsamość git (`user.name=Orkiestrator`) **tylko gdy żadnej nie ma**, lokalnej ani globalnej · ustawia `core.hooksPath` na `.githooks`, co **nadpisuje istniejącą ścieżkę hooków** · uruchamia `npm ci`, co **kasuje i instaluje na nowo `node_modules`** z lockfile'a · uruchamia `weryfikacja.js` jako sprawdzenie tylko do odczytu.
...
HEXAGRAM wyrósł z trzech warstw. Dyscyplina opisana wyżej to trzecia; dwie pierwsze są powodem, dla którego system w ogóle zajmuje się czasem.
