...
# wpisz same domeny prywatne. Zapisane niżej z pojedynczym znakiem w nawiasie —
# analogicznie do `P[E]SEL`/`N[I]P` poniżej — celowo: regex łapie domenę
# dosłownie, ale ten SZABLON (który wychodzi w odlewie) sam siebie nie łapie.
@gm[a]il
@w[p]\.
@on[e]t
@prot[o]n
...
ul\. 
[Uu]lica 
# 13.09.2026, zmierzone `straz_czystosci.sh` na własnym ciele: bez kotwicy `\<` wzorzec
# łapał podciąg liczby z WIĘKSZĄ liczbą cyfr z przodu (numeryczny zakres w komentarzu kodu,
# jednostka czasu jako litera po spacji), bo `-i` unieważnia [A-Z] jako sygnał kontekstu
# (dopasowuje też małe litery). Kotwica wymaga, by dwie pierwsze cyfry NIE były poprzedzone
# inną cyfrą — realny kod pocztowy przechodzi, przypadkowy fragment dłuższej liczby — nie.
